...
const MovingImg = ({title,img,link})=>{
    ...
    return(
        <Link href={link} target="_blank" onMouseMove={handleMouse} onMouseLeave={handleMouseLeave}>
            ...
            <FramerImage style={{x:x, y:y}} whileInView={{opacity:1,transition:{duration:0.2}}} ref={imgRef} src={img} alt={title} className='w-96 z-10 h-auto hidden absolute rounded-lg md:!hidden'/>
        </Link>
    )
}
    ...
    return(
        <li className='col-span-1 w-full p-4 relative bg-light border border-solid border-dark rounded-2xl dark:bg-dark dark:border-light'>
            ...
            <Link href={link} target='_blank' className='w-full cursor-pointer overflow-hidden rounded-lg inline-block '>
                <FramerImage src={img} alt={title} className='w-full h-auto' whileHover={{scale:1.05}}transition={{duration:.2}} priority sizes="(max-width:786px) 100vw, (max-width:1200px) 50vw, 50vw "/>
            </Link>
...
const articles = () => {
  ...
  return (
    <>
    <Head>
        <title>Shrey | Articles Page</title>
